package GUS::Model::EDA_UD::AttributeGraph_Table;

# THIS CLASS HAS BEEN AUTOMATICALLY GENERATED BY THE GUS::ObjRelP::Generator 
# PACKAGE.
#
# DO NOT EDIT!!


use strict;
use GUS::ObjRelP::DbiTable;
use vars qw (@ISA);
@ISA = qw (GUS::ObjRelP::DbiTable);

sub setDefaultParams {
  my $self = shift;
  $self->setChildList();

  $self->setParentList(
      ['GUS::Model::SRes::OntologyTerm','ontology_term_id','ontology_term_id'],
      ['GUS::Model::SRes::OntologyTerm','parent_ontology_term_id','ontology_term_id'],
      ['GUS::Model::EDA_UD::Study','study_id','study_id'],
      ['GUS::Model::Core::AlgorithmInvocation','row_alg_invocation_id','algorithm_invocation_id'],
      ['GUS::Model::Core::GroupInfo','row_group_id','group_id'],
      ['GUS::Model::Core::UserInfo','row_user_id','user_id'],
      ['GUS::Model::Core::ProjectInfo','row_project_id','project_id']);

  $self->setAttributeNames(
      'attribute_graph_id',
      'study_id',
      'ontology_term_id',
      'stable_id',
      'parent_stable_id',
      'parent_ontology_term_id',
      'provider_label',
      'display_name',
      'display_order',
      'definition',
      'display_type',
      'hidden',
      'display_range_min',
      'display_range_max',
      'is_merge_key',
      'impute_zero',
      'is_repeated',
      'bin_width_override',
      'is_temporal',
      'is_featured',
      'ordinal_values',
      'modification_date',
      'user_read',
      'user_write',
      'group_read',
      'group_write',
      'other_read',
      'other_write',
      'row_user_id',
      'row_group_id',
      'row_project_id',
      'row_alg_invocation_id');

  $self->setQuotedAtts(
      'provider_label',
      'stable_id',
      'parent_stable_id',
      'display_name',
      'display_type',
      'definition',
      'display_range_min',
      'modification_date',
      'bin_width_override',
      'display_range_max',
      'hidden',
      'ordinal_values');

  $self->setAttInfo(
      {'col' => 'attribute_graph_id', 'type' => 'NUMBER', 'prec' => 12, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'study_id', 'type' => 'NUMBER', 'prec' => 12, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'ontology_term_id', 'type' => 'NUMBER', 'prec' => 10, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'stable_id', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 255, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'parent_stable_id', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 255, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'parent_ontology_term_id', 'type' => 'NUMBER', 'prec' => 10, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'provider_label', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 3200, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'display_name', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 1500, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'display_order', 'type' => 'NUMBER', 'prec' => 3, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'definition', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 4000, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'display_type', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 20, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'hidden', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 64, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'display_range_min', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 16, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'display_range_max', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 16, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'is_merge_key', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'impute_zero', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'is_repeated', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'bin_width_override', 'type' => 'VARCHAR2', 'prec' => '', 'length' => 16, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'is_temporal', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'is_featured', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'ordinal_values', 'type' => 'CLOB', 'prec' => '', 'length' => 4000, 'Nulls' => 1, 'base_type' => 'not set'}, 
      {'col' => 'modification_date', 'type' => 'DATE', 'prec' => '', 'length' => 7, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'user_read', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'user_write', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'group_read', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'group_write', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'other_read', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'other_write', 'type' => 'NUMBER', 'prec' => 1, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'row_user_id', 'type' => 'NUMBER', 'prec' => 12, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'row_group_id', 'type' => 'NUMBER', 'prec' => 3, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'row_project_id', 'type' => 'NUMBER', 'prec' => 4, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'}, 
      {'col' => 'row_alg_invocation_id', 'type' => 'NUMBER', 'prec' => 12, 'length' => 22, 'Nulls' => 0, 'base_type' => 'not set'});

  $self->setRealTableName('GUS::Model::EDA_UD::AttributeGraph');

  $self->setIsView(0);

  $self->setParentRelations(
      ['GUS::Model::SRes::OntologyTerm','ontology_term_id','ontology_term_id'], 
      ['GUS::Model::SRes::OntologyTerm','parent_ontology_term_id','ontology_term_id'], 
      ['GUS::Model::EDA_UD::Study','study_id','study_id'], 
      ['GUS::Model::Core::AlgorithmInvocation','row_alg_invocation_id','algorithm_invocation_id'], 
      ['GUS::Model::Core::GroupInfo','row_group_id','group_id'], 
      ['GUS::Model::Core::UserInfo','row_user_id','user_id'], 
      ['GUS::Model::Core::ProjectInfo','row_project_id','project_id']);

  $self->setChildRelations();

  $self->setHasSequence(0);

  $self->setPrimaryKeyList('attribute_graph_id');

}

1;

